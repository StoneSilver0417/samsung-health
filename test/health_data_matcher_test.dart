...
void main() {
  group('HealthDataMatcher In-Memory Matching', () {
    ...
    test(
      'matchHrSamples extracts only samples within time range and source',
      () {
        final allHrPoints = [
          _point(
            uuid: 'hr-before',
    ...
    test(
      'matchDistanceDeltas filters and sorts deltas within session interval',
      () {
        final allDistPoints = [
          _point(
            uuid: 'dist-in-2',
    ...
    test(
      'matchCalories favors workoutTotalCalories then falls back to summing records',
      () {
        final allCalPoints = [
          _point(
            uuid: 'cal-1',
    ...
    test(
      'matchSteps follows priority: workout totalSteps > nativeSteps > health STEPS',
      () {
    ...
    test(
      'buildSegmentsAndLaps builds intervals and watch laps with distance and avgHr',
      () {
        final deltas = [
          DistDelta(
            from: start,
    ...
    test('buildRunSession constructs complete RunSession in memory', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'A whole-session distance aggregate must not become fake splits',
      );
      expect(session.hrSeries, isNotEmpty);
      expect(session.sourceName, sourceName);
    });

    test('computeSplits requires a granular distance time series', () {
      final aggregateOnly = HealthDataMatcher.computeSplits(start, [
        DistDelta(from: start, to: end, meters: 4500),
      ], const []);
      expect(aggregateOnly, isEmpty);

      final granular = HealthDataMatcher.computeSplits(start, [
        DistDelta(
          from: start,
          to: start.add(const Duration(minutes: 6)),
          meters: 1000,
        ),
        DistDelta(
          from: start.add(const Duration(minutes: 6)),
          to: start.add(const Duration(minutes: 11, seconds: 30)),
          meters: 1000,
        ),
      ], const []);
      expect(granular, hasLength(2));
      expect(granular[0].paceSecPerKm, 360);
      expect(granular[1].paceSecPerKm, 330);
    });
  });
}
