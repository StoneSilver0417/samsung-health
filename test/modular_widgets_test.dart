...
void main() {
  ...
  group('Analysis Sub-Widgets Isolated Tests', () {
    testWidgets('MonthlySummaryCard & MonthCompareCard render metrics and comparisons',
        (tester) async {
      final runs = [
        _sampleSession(id: 'r1', startTime: DateTime(2026, 8, 1, 20), km: 10.0, durationSec: 3600),
        _sampleSession(id: 'r2', startTime: DateTime(2026, 7, 15, 20), km: 8.0, durationSec: 3000),
      ];
    ...
    testWidgets('MonthCompareCard shows fallback when no last month runs', (tester) async {
      final runs = [
    ...
    testWidgets('PaceTrendChartCard shows notice when < 2 runs', (tester) async {
      final runs = [
        _sampleSession(id: 'r1', startTime: DateTime.now(), km: 5.0),
      ];
    ...
    testWidgets('PaceTrendChartCard renders chart when >= 2 runs', (tester) async {
      final runs = [
        _sampleSession(id: 'r1', startTime: DateTime(2026, 8, 1, 20), km: 5.0, durationSec: 1800),
        _sampleSession(id: 'r2', startTime: DateTime(2026, 8, 3, 20), km: 5.0, durationSec: 1700),
      ];
    ...
    testWidgets('Vo2maxTrendChartCard renders latest metric and chart', (tester) async {
      final series = [
    ...
    testWidgets('PbRecordsCard renders personal best records correctly', (tester) async {
      final runs = [
        _sampleSession(id: 'r1', startTime: DateTime(2026, 8, 1, 20), km: 10.0, durationSec: 3600),
      ];
      ...
      await tester.pumpWidget(
        _wrapWidget(child: PbRecordsCard(stats: stats)),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('GoalRecommendCard displays recommendations or guide', (tester) async {
      final repo = InMemoryRunRepository();
      await repo.saveGoalRecommendation('5km 25분 목표 추천', DateTime(2026, 8, 1, 12));
  ...
  group('RunDetail Sub-Widgets Isolated Tests', () {
    testWidgets('RunDetailHeroHeader and MetricsGrid render all session details', (tester) async {
    ...
    testWidgets('RunDetailAiSummaryCard renders cached summary or guide', (tester) async {
      final repo = InMemoryRunRepository();
      await repo.saveAiSummary('rd_ai', '우수한 인터벌 트레이닝이었습니다.');
    ...
    testWidgets('RunDetailRunningDynamicsCard displays stride and drift indicators', (tester) async {
      final run = _sampleSession(
        id: 'rd2',
        km: 5.0,
        durationSec: 1800,
    ...
    testWidgets('RunDetailHrChartCard and HrZonesCard render HR chart and Z1~Z5 zones', (tester) async {
      final run = _sampleSession(
        id: 'rd3',
        avgHr: 150,
        maxHr: 170,
      ...
      expect(find.bySemanticsLabel('심박수 시계열 차트: 평균 150bpm, 최고 170bpm'), findsOneWidget);
      expect(find.text('Z1'), findsOneWidget);
      expect(find.text('Z5'), findsOneWidget);
    });

    testWidgets('RunDetailLapsTableCard and SegmentsCard render laps and intervals', (tester) async {
      final run = _sampleSession(
        id: 'rd4',
        laps: [
          RunLap(
    ...
    testWidgets('RunDetailDeleteDialog shows confirm dialog and returns decision', (tester) async {
      bool? result;
