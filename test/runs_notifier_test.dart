...
class _FakeGeminiService extends GeminiService {
  final List<String> analyzedRunIds = [];
  final Map<String, List<String>> recentRunIds = {};
  Object? error;
  ...
  Future<String> summarizeRun(
    ...
    recentRunIds[run.id] = recentRuns.map((item) => item.id).toList();
    return 'cached analysis for ${run.id}';
  }
}
...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [
        ...
        nowProvider.overrideWithValue(() => DateTime(2026, 8, 12, 22)),
      ],
    );
  });
  ...
  group('RunsNotifier Initialization (build)', () {
    ...
    test(
      'initializes with sorted runs from repository (descending by startTime)',
      () async {
  ...
  group('RunsNotifier.sync', () {
    test(
      'successfully syncs health data, updates VO2max, sets sync timestamp and evaluates achievements',
      () async {
    ...
    test(
      'filters out ignoredIds during sync so deleted or ignored runs are not re-added',
      () async {
        final run1 = _sampleRun(
          id: 'ignore-1',
    ...
    test(
      'does not overwrite VO2max series when health service returns empty VO2 data',
      () async {
    ...
    test(
      'returns error SyncResult when Health Connect permission is denied',
      () async {
    ...
    test(
      'handles exceptions during sync gracefully and returns error SyncResult',
      () async {
        ...
        expect(
          result.error,
          contains('동기화 실패: Bad state: Health Connect connection failed'),
        );
        expect(result.addedCount, 0);
      },
    );

    test('default sync never starts automatic AI analysis', () async {
      repo.apiKey = 'configured-key';
      health.runsToReturn = [
        _sampleRun(id: 'today', startTime: DateTime(2026, 8, 12, 20)),
      ];

      final result = await container.read(runsProvider.notifier).sync();

      expect(result.addedCount, 1);
      expect(result.aiAnalyzedCount, 0);
      expect(gemini.analyzedRunIds, isEmpty);
      expect(repo.getAiSummary('today'), isNull);
    });

    test(
      'opt-in sync skips automatic AI analysis when API key is blank',
      () async {
        repo.apiKey = '   ';
        health.runsToReturn = [
          _sampleRun(id: 'today', startTime: DateTime(2026, 8, 12, 20)),
        ...
        final result = await container
            ...
            .sync(autoAnalyzeNewTodayRuns: true);

        expect(result.addedCount, 1);
        expect(result.aiAnalyzedCount, 0);
        expect(result.aiAnalysisError, isNull);
        expect(gemini.analyzedRunIds, isEmpty);
      },
    );

    test(
      'opt-in sync analyzes and caches only newly added runs from today',
      () async {
        repo.apiKey = ' configured-key ';
        final recent = _sampleRun(
          id: 'recent-existing',
          startTime: DateTime(2026, 8, 11, 20),
        );
        await repo.upsertAll([recent]);
        health.runsToReturn = [
          recent,
          _sampleRun(id: 'today', startTime: DateTime(2026, 8, 12, 20)),
          _sampleRun(id: 'old-new', startTime: DateTime(2026, 8, 10, 20)),
        ];
        ...
        expect(result.addedCount, 2);
        expect(result.aiAnalyzedCount, 1);
        expect(result.aiAnalysisError, isNull);
        expect(gemini.analyzedRunIds, ['today']);
        expect(gemini.recentRunIds['today'], contains('recent-existing'));
        expect(gemini.recentRunIds['today'], isNot(contains('today')));
        expect(repo.getAiSummary('today'), 'cached analysis for today');
        expect(repo.getAiSummary('old-new'), isNull);
      },
    );

    test('opt-in sync does not replace an existing nonblank cache', () async {
      repo.apiKey = 'configured-key';
      await repo.saveAiSummary('today', 'existing analysis');
      health.runsToReturn = [
        _sampleRun(id: 'today', startTime: DateTime(2026, 8, 12, 20)),
      ];
      ...
      expect(result.aiAnalyzedCount, 0);
      expect(gemini.analyzedRunIds, isEmpty);
      expect(repo.getAiSummary('today'), 'existing analysis');
    });

    test(
      'AI generation failure is surfaced without failing successful sync',
      () async {
        repo.apiKey = 'configured-key';
        gemini.error = StateError('quota exceeded');
        health.runsToReturn = [
          _sampleRun(id: 'today', startTime: DateTime(2026, 8, 12, 20)),
        ];
        ...
        expect(result.addedCount, 1);
        expect(result.aiAnalyzedCount, 0);
        expect(result.aiAnalysisError, contains('today'));
        expect(result.aiAnalysisError, contains('quota exceeded'));
        expect(container.read(runsProvider).value, hasLength(1));
      },
    );
  });
  ...
  group('RunsNotifier.fetchCandidates', () {
    test(
      'fetches candidates within 30 days without requesting history permission',
      () async {
        final from = DateTime.now().subtract(const Duration(days: 10));
        final candidate1 = _sampleRun(
          id: 'c1',
          startTime: DateTime.now().subtract(const Duration(days: 5)),
        );
        final candidate2 = _sampleRun(
          id: 'c2',
    ...
    test(
      'requests history permission when from date is older than 29 days',
      () async {
        final from = DateTime.now().subtract(const Duration(days: 60));
        health.runsToReturn = [
          _sampleRun(
            id: 'c-old',
            startTime: DateTime.now().subtract(const Duration(days: 45)),
          ),
        ];
    ...
    test(
      'throws Exception when base permission is denied during candidate fetching',
      () async {
        health.permissionGranted = false;
    ...
    test(
      'throws Exception when history permission is denied for > 29 days query',
      () async {
  ...
  group('RunsNotifier.importRuns', () {
    test(
      'imports selected runs, manages ignored IDs, updates sync time, and triggers badge evaluation',
      () async {
        ...
        expect(
          state.map((r) => r.id),
          containsAll(['run-to-reselect', 'new-run']),
        );
      },
    );
  });
  ...
  group('RunsNotifier.deleteRun', () {
    test(
      'deletes run from repository, adds id to ignoredIds, and updates state',
      () async {
  ...
  group('RunsNotifier.seedDemoData and clearAll', () {
    test(
      'seedDemoData generates 12 demo runs, evaluates badges, and populates state',
      () async {
        final notifier = container.read(runsProvider.notifier);
    ...
    test(
      'clearAll clears repository data and updates state to empty list',
      () async {
  ...
  group('MutationQueue Integration and Concurrency', () {
    test(
      'executes concurrent mutation calls in strict sequential FIFO order',
      () async {
        final order = <String>[];
        ...
        final op1 = notifier
            .importRuns([_sampleRun(id: 'r1', startTime: DateTime(2026, 8, 1))])
    ...
    test(
      'continues subsequent mutations even if a prior mutation in queue throws an error',
      () async {
  ...
  group('Derived Providers Reactivity', () {
    test(
      'statsProvider, vo2SeriesProvider, and earnedBadgesProvider update on state changes',
      () async {
        final run1 = _sampleRun(
          id: 'r1',
