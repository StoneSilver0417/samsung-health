...
void main() {
  group('Gemini Typed DTOs', () {
    test('GeminiPartDto serializes and deserializes correctly', () {
      final partWithThought =
          const GeminiPartDto(text: 'Thinking...', thought: true);
    ...
    test('GeminiContentDto and GeminiGenerationConfigDto conversions', () {
      ...
      expect(
        GeminiGenerationConfigDto.fromJson({'maxOutputTokens': 512})
            .maxOutputTokens,
        512,
      );
    });

    test('GeminiGenerateRequestDto builds text prompt and serializes to JSON',
        () {
      ...
      expect(map['contents'][0]['parts'][0]['text'],
          'Generate workout advice');
      expect(map['generationConfig']['maxOutputTokens'], 2048);

      final fromJson = GeminiGenerateRequestDto.fromJson(map);
      expect(fromJson.contents.first.parts.first.text,
          'Generate workout advice');
      expect(fromJson.generationConfig.maxOutputTokens, 2048);
    });

    test('GeminiGenerateResponseDto extracts clean text and ignores thought: true',
        () {
    ...
    test('GeminiGenerateResponseDto throws when candidates list is empty or text is blank',
        () {
  ...
  group('GeminiPromptBuilder', () {
    ...
    test('buildRunSummaryPrompt and buildGoalRecommendationPrompt structure', () {
